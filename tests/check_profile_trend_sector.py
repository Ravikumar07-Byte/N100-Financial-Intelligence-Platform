...
for name in known_companies:
    ...
    match = companies[
        companies["company_name"].str.lower()
        == name.lower()
    ]
    ...
    if len(match) == 1:
        ...
        ok(
            f"{name} found | "
            f"Sector metadata available through sectors table"
        )

    elif len(match) > 1:

        fail(
            f"{name}: multiple company records found."
        )

    else:

        warn(
            f"{name}: exact company name not found."
        )
...
if invalid_result.empty:
    ...
    ok(
        "Invalid company returns zero database records."
    )

else:

    fail(
        "Invalid company unexpectedly matched a record."
    )
...
if invalid_result.empty:
    ...
    ok(
        "Invalid company lookup does not require "
        "a fabricated company record."
    )
...
if trend_company.empty:
    ...
    available_years = int(
        trend_company.iloc[0]["years_available"]
    )

    print(
        f"Trend test company_id : {trend_company_id}"
    )

    print(
        f"Available P&L records  : {available_years}"
    )

    if available_years >= 10:
        ok(
            "At least 10 historical P&L records "
            "are available for trend analysis."
        )
    else:
        warn(
            "Less than 10 historical P&L records "
            "are available."
        )
...
if not trend_company.empty:
    ...
    if "year" in trend_df.columns:
        ...
        if len(unique_years) >= 10:
            ...
            ok(
                f"Trend dataset contains {len(unique_years)} "
                "unique years."
            )
            ...
            warn(
                f"Only {len(unique_years)} unique years found."
            )
...
if not trend_company.empty:
    ...
    if not current.empty:
        ...
        warn(
            f"No P&L record found for {YEAR} "
            f"for test company."
        )
...
if revenue_column:
    ...
    if valid_revenue >= 2:
        ...
        ok(
            f"Revenue trend data available "
            f"({valid_revenue} records)."
        )
    ...
    warn(
        "Could not automatically identify revenue column."
    )
...
if profit_column:
    ...
    if valid_profit >= 2:
        ...
        ok(
            f"Net profit trend data available "
            f"({valid_profit} records)."
        )

    else:

        fail(
            "Insufficient net profit history."
        )

else:

    warn(
        "Could not automatically identify net profit column."
    )
...
if (
    ...
    if valid_yoy >= 1:
        ...
        ok(
            f"YoY growth can be calculated "
            f"for {valid_yoy} year transitions."
        )

    else:

        fail(
            "YoY calculation produced no valid transitions."
        )
...
if sector_counts.empty:
    ...
    ok(
        f"Broad-sector classification exists "
        f"for {len(sector_counts)} sectors."
    )
...
missing_sector = sectors[
    sectors["broad_sector"].isna()
]
...
if missing_sector.empty:
    ...
    fail(
        f"{len(missing_sector)} companies have "
        "missing broad-sector classification."
    )
...
missing_subsector = sectors[
    sectors["sub_sector"].isna()
]
...
if missing_subsector.empty:
    ...
    warn(
        f"{len(missing_subsector)} companies have "
        "missing sub-sector classification."
    )
...
if sector_company_ids == company_count:
    ...
    ok(
        "Every company has a sector record."
    )

else:

    fail(
        f"Sector coverage mismatch: "
        f"{sector_company_ids}/{company_count} companies."
    )
...
if ratios.empty:
    ...
    fail(
        f"No financial ratio data found for {YEAR}."
    )

else:

    ok(
        f"Financial ratio data available for {YEAR}."
    )
...
if len(found_kpis) >= 2:
    ...
    ok(
        f"{len(found_kpis)} sector-analysis KPI fields detected."
    )

else:

    warn(
        "Fewer than 2 expected KPI fields "
...
if "company_id" not in ratios.columns:
    ...
    print(
        f"Joined sector + ratio rows: {len(merged)}"
    )

    if merged.empty:

        fail(
            "Sector and financial-ratio tables "
            "could not be joined."
        )
        ...
        ok(
            "Sector and financial-ratio data "
            "can be joined."
        )
        ...
        for label, column in found_kpis.items():
            ...
            if not median_df.empty:
                ...
                ok(
                    f"{label}: median calculated "
                    f"for {len(median_df)} sectors."
                )
                ...
                fail(
                    f"{label}: no valid sector medians."
                )
...
if subsector_counts.empty:
    ...
    ok(
        f"{len(subsector_counts)} sector/sub-sector "
        "groups detected."
    )
...
print(
    f"Materials companies: {len(materials)}"
)
...
if materials.empty:
    ...
    fail(
        "Materials sector is missing."
    )

else:

    ok(
        f"Materials sector contains "
        f"{len(materials)} companies."
    )
...
print(
    f"Information Technology companies: {len(it)}"
)
...
if it.empty:
    ...
    fail(
        "Information Technology sector is missing."
    )

else:

    ok(
        f"Information Technology sector contains "
        f"{len(it)} companies."
    )
...
if duplicates.empty:
    ...
    ok(
        "Each company has exactly one sector assignment."
    )

else:

    warn(
        f"{len(duplicates)} duplicate sector records found."
    )
...
if "market_cap_category" in sectors.columns:
    ...
    if missing_market_cap == 0:
        ...
        ok(
            "No missing market-cap categories."
        )

    else:

        warn(
            f"{missing_market_cap} missing "
...
if FAIL == 0 and WARN == 0:
    ...
    print(
        "  [PASS] Profile + Trend + Sector verification clean."
    )
